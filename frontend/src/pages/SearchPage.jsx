...
const SearchPage = () => {
    ...
    const handleSearch = async (e) => {
        e.preventDefault();
        try{
            const res = await axios.get(`https://netflixclone-qsk0.onrender.com/api/v1/search/${activeTab}/${searchTerm}`);
            setResults(res.data.content);
        }catch(error){
            if(error.response.status == 404){
                toast.error("Nothing found, make sure you are searching under right category");
            }else{
                        ...
                        <div key={result.id} className="bg-gray-800 p-4 rounded">
                            {activeTab === 'person' ? (
                                ...
                                <Link to={"/watch/" + result.id} onClick={()=>{setContentType(activeTab);
                                    ...
                                    <h2 className="text-center mt-2">{result.title || result.name}</h2>
                                </Link>
                            )}
                        </div>
                    )
